...
describe("root node", () => {
	describe("default match", () => {
		test("match", () => {
			assert.strictEqual(queryAll(<div />, {}).length, 1);
		});
	...
	describe("custom match", () => {
		test("match", () => {
			...
			assert.notStrictEqual(match.mock.calls[0], [element, query]);
		});
...
describe("child node", () => {
	...
	describe("custom match", () => {
		test("match", () => {
			...
			assert.strictEqual(
				queryAll(<main>{element}</main>, query, {
					match,
				}).length,
				1,
...
test("multiple results", () => {
	const res1 = <div className="foo bar" />;

	const res2 = <span className="bar baz" />;
...
test("slice results with limit param", () => {
	const res1 = <div className="foo" />;

	const res2 = <span className="foo" />;

	const notRes3 = <span className="foo" />;
